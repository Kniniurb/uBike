...
public class AntSpeedDevice extends AntDevice {
	private static final String CLASSTAG = AntSpeedDevice.class.getSimpleName();

	private int prevEventTime = Integer.MIN_VALUE;
	private int prevRevCount = Integer.MIN_VALUE;
	private long lastCall = 0;

	private double revs = 0;

//	public AntSpeedDevice(final AntManager mrg, final Context context, final int sub) {
//		super(mrg, DeviceType.SPEED_SENSOR, AntDeviceProfile.BIKESPEED, sub, EnumSet.of(ValueType.WHEEL_REVS, ValueType.SPEED));
//	}
	
	public AntSpeedDevice(AntMesgCallback callback) {
		super(AntProfile.BIKESPEED, callback);
	}
	...
	public boolean decode(byte[] antMsg) {
		...
		if (now - lastCall > ANT_MSG_TIMEOUT) {
			...
			prevRevCount = Integer.MIN_VALUE;
		}
		lastCall = now;
		...
		if (revCount < prevRevCount) prevRevCount -= 65536;
		if (eventTime > prevEventTime  && revCount > prevRevCount && prevRevCount != Integer.MIN_VALUE) {
			revs = 60.0 * 1024 * (revCount - prevRevCount) / (eventTime - prevEventTime);
			fireOnDeviceValueChanged(ValueType.WHEEL_REVS, revs, now);
			fireOnDeviceValueChanged(ValueType.SPEED, revs * 0.001 * SenseGlobals.wheelCirc / 60.0, now);
			rawValue = Format.format(revs * 0.001 * SenseGlobals.wheelCirc / 60.0, 1) + " m/s";

			//LLog.d(Globals.TAG, CLASSTAG + ".decode: EventTime = " + eventTime + " " + prevEventTime + ", RevCount = " + revCount + " " + prevRevCount  + ", Revs = " + revs);				
		}
		//LLog.d(Globals.TAG, CLASSTAG + ".decode: EventTime = " + eventTime + " " + prevEventTime + ", RevCount = " + revCount + " " + prevRevCount  + ", Revs = " + revs);				
		
		prevRevCount = revCount;
	...
	public void fadeOut(ValueType type) {
		long now = System.currentTimeMillis();
		if (type.equals(ValueType.WHEEL_REVS) && revs != Double.NaN && Math.abs(revs) > 1.0) {
